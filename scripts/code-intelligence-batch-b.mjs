...
const OUTPUT = 'evals/code-intelligence/results/phase2-batch-b.json';
...
const temporary = await mkdtemp(path.join(os.tmpdir(), 'memory-recall-batch-b-fixtures-'));

try {
  const nativeBinary = path.resolve(root, 'rust', 'target', 'release', process.platform === 'win32' ? 'oaf.exe' : 'oaf');
  if (!(await stat(nativeBinary)).isFile()) throw new Error('batch_b_native_release_binary_missing');
  ...
  for (const [id, scope] of mode === 'fixtures' ? [] : Object.entries(REPOSITORY_SCOPES)) {
    ...
    if (!repository) throw new Error(`batch_b_repository_missing:${id}`);
  ...
  const languages = ['python', 'go', 'rust'].map((language) => aggregateLanguage(language, cases, gates.languageFull));
  ...
  if (mode === 'fixtures') {
    ...
    if (failed.length > 0) throw new Error('batch_b_fixture_truth_failed');
    console.log(`Batch B fixture truth passed: ${cases.length} cases.`);
  } else if (mode === 'check') {
    const stored = await readJson(OUTPUT);
    if (JSON.stringify(comparableReport(stored)) !== JSON.stringify(comparableReport(report))) {
      throw new Error('batch_b_evidence_stale');
    }
    if (stored.reportFingerprint !== fingerprint(comparableReport(stored))) {
      throw new Error('batch_b_report_fingerprint_invalid');
    }
    console.log(`Batch B evidence is current: ${cases.length} cases, ${languages.length} languages.`);
  } else {
    await atomicWrite(OUTPUT, report);
    console.log(`Batch B evaluated ${cases.length} cases and wrote ${OUTPUT}.`);
  }
} finally {
...
async function runCase(definition, provider) {
  if (definition.truth.source.ref !== definition.sourceRef) throw new Error(`batch_b_truth_source_mismatch:${definition.id}`);
  ...
  if (before !== after) throw new Error(`batch_b_workspace_changed:${definition.id}`);
...
async function materializeFixtures(_directory) {
  return Promise.all(['python', 'go', 'rust'].map((language) => {
    const fixtureRoot = path.resolve(root, 'evals', 'code-intelligence', 'fixtures', 'batch-b', language);
    return fixtureDefinition(language, fixtureRoot, `fixture_${language}_batch_b`);
  }));
}
...
async function ensurePinnedRepository(repository) {
  ...
  if (commit !== repository.commit) throw new Error(`batch_b_repository_commit_mismatch:${repository.id}`);
  return directory;
}
...
function parseMode(args) {
  ...
  throw new Error('usage: node scripts/code-intelligence-batch-b.mjs [--check|--fixtures-only]');
}
