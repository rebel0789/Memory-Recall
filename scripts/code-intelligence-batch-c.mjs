...
const OUTPUT = 'evals/code-intelligence/results/phase2-batch-c.json';
...
const temporary = await mkdtemp(path.join(os.tmpdir(), 'memory-recall-batch-c-fixtures-'));

try {
  const nativeBinary = path.resolve(root, 'rust', 'target', 'release', process.platform === 'win32' ? 'oaf.exe' : 'oaf');
  if (!(await stat(nativeBinary)).isFile()) throw new Error('batch_c_native_release_binary_missing');
  ...
  for (const [id, scope] of mode === 'fixtures' ? [] : Object.entries(REPOSITORY_SCOPES)) {
    ...
    if (!repository) throw new Error(`batch_c_repository_missing:${id}`);
  ...
  const languages = ['java', 'kotlin', 'csharp'].map((language) => aggregateLanguage(language, cases, gates.languageFull));
  ...
  if (mode === 'fixtures') {
    ...
    if (failed.length > 0) throw new Error('batch_c_fixture_truth_failed');
    console.log(`Batch C fixture truth passed: ${cases.length} cases.`);
  } else if (mode === 'check') {
    const stored = await readJson(OUTPUT);
    if (JSON.stringify(comparableReport(stored)) !== JSON.stringify(comparableReport(report))) {
      throw new Error('batch_c_evidence_stale');
    }
    if (stored.reportFingerprint !== fingerprint(comparableReport(stored))) {
      throw new Error('batch_c_report_fingerprint_invalid');
    }
    console.log(`Batch C evidence is current: ${cases.length} cases, ${languages.length} languages.`);
  } else {
    await atomicWrite(OUTPUT, report);
    console.log(`Batch C evaluated ${cases.length} cases and wrote ${OUTPUT}.`);
  }
} finally {
...
async function runCase(definition, provider) {
  if (definition.truth.source.ref !== definition.sourceRef) throw new Error(`batch_c_truth_source_mismatch:${definition.id}`);
  ...
  if (before !== after) throw new Error(`batch_c_workspace_changed:${definition.id}`);
...
async function materializeFixtures(_directory) {
  return Promise.all(['java', 'kotlin', 'csharp'].map((language) => {
    const fixtureRoot = path.resolve(root, 'evals', 'code-intelligence', 'fixtures', 'batch-c', language);
    return fixtureDefinition(language, fixtureRoot, `fixture_${language}_batch_c`);
  }));
}
...
async function ensurePinnedRepository(repository) {
  ...
  if (commit !== repository.commit) throw new Error(`batch_c_repository_commit_mismatch:${repository.id}`);
  return directory;
}
...
function parseMode(args) {
  ...
  throw new Error('usage: node scripts/code-intelligence-batch-c.mjs [--check|--fixtures-only]');
}
