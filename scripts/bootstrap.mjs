...
console.log(`Memory Recall ${pkg.version} source bootstrap is ready.`);console.log('');console.log('Local state: .local/state.json');console.log('Native memory: .local/memory.sqlite (created on first use)');console.log('Artifacts: .local/artifacts');console.log('Model mode: deterministic (no key, no network)');console.log('External writes: disabled');console.log('');console.log('Next:');console.log('  npm run doctor');console.log('  npm run ci');console.log('  npm run recall -- skill catalog --read-only --root . --format summary');console.log('  docs/usage/local-agent-handoff.md');console.log('  npm run dev');console.log('  open http://127.0.0.1:4310');
