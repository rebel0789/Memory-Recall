...
- Tier 1 documentation now names the 61 capability rows that meet the sampled Phase 2 floor, the 92 applicable rows that remain unmeasured, and the single not-applicable row. Reviewed import evidence preserves full coordinates across Java, Go, Rust, Kotlin, and C# fixtures plus three pinned repositories per language. Every Tier 1 language remains unmeasured overall; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
