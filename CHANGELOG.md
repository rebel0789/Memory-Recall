...
- Tier 1 documentation now names the 66 capability rows that meet the sampled Phase 2 floor, the 87 applicable rows that remain unmeasured, the zero recorded floor failures, and the single not-applicable row. Reviewed import evidence now also covers PHP, Ruby, Swift, C, and C++ fixtures plus three pinned repositories per language. Grouped PHP imports remain unsupported and unmeasured. Every Tier 1 language remains unmeasured overall; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
