...
- Tier 1 documentation now names the 74 capability rows that meet the sampled Phase 2 floor, the 79 applicable rows that remain unmeasured, the zero recorded floor failures, and the single not-applicable row. Go, Rust, and Dart exports meet the sampled fixture-plus-three-repository floor. Dart calls meet the same sampled floor, including exact `BookstoreAuth.of(context)` resolution and a same-name `GoRouter.of(context)` decoy. Dart heritage now also meets the sampled floor: Flutter retains only the direct outer `State` superclass for `_BookstoreState`, rejects its `Bookstore` generic argument as heritage, and keeps the sampled mixin; Shelf keeps `RouterParams on Request` explicitly unresolved; HTTP resolves `BaseClient implements Client` exactly. Kotlin calls and types and C++ types retain their reviewed sampled floors. Generic substitution, compiler-equivalent analysis, grouped PHP imports, and broader framework or monorepo resolution remain unmeasured. Every Tier 1 language remains unmeasured overall; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
