...
- Tier 1 documentation now names the 51 capability rows that meet the sampled Phase 2 floor and the 103 rows that remain unmeasured or not applicable. TypeScript exact-import, export, and type-inference evidence now meet the sampled floor across its fixture and all three pinned repositories. TypeScript and JavaScript are the first two languages whose Phase 2 applicable sampled rows all meet the floor; TypeScript heritage, configuration, frameworks, impact, and processes remain unevaluated, the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
