...
- Tier 1 documentation now names the 48 capability rows that meet the sampled Phase 2 floor and the 106 rows that remain unmeasured or not applicable. JavaScript is the first language whose applicable sampled rows all meet the floor; the other thirteen remain unmeasured, the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
