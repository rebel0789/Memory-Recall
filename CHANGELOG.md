...
- Stable npm publication is now native-first and fail-closed: one exact successful Rust run must supply all five checksum-verified packages, file-complete SPDX 2.3 SBOMs, and signed GitHub provenance. Every native registry integrity and both npm publish/SLSA attestations must verify before the root package can publish. The lane remains local and unproven until the remote five-platform and final release gates pass.
