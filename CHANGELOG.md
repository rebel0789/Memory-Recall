...
- Tier 1 documentation now names the 49 capability rows that meet the sampled Phase 2 floor and the 105 rows that remain unmeasured or not applicable. TypeScript export evidence now meets the sampled floor across its fixture and all three pinned repositories. JavaScript remains the first language whose applicable sampled rows all meet the floor; the other thirteen remain unmeasured, the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
