...
- Tier 1 documentation now names the 54 capability rows that meet the sampled Phase 2 floor and the 100 rows that remain unmeasured or not applicable. TypeScript exact-import, export, and type-inference evidence plus Python exact-import, reviewed sampled heritage, and exact reviewed construction evidence meet the floor across each language's fixture and all three pinned repositories. Python configuration and frameworks remain unmeasured. TypeScript and JavaScript are still the only languages whose Phase 2 applicable sampled rows all meet the floor; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
