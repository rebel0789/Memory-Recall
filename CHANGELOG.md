...
- Tier 1 documentation now names the 73 capability rows that meet the sampled Phase 2 floor, the 80 applicable rows that remain unmeasured, the zero recorded floor failures, and the single not-applicable row. Go, Rust, and Dart exports meet the sampled fixture-plus-three-repository floor. Go evidence omits exported fields and interface methods; Rust exact evidence is limited to unrestricted top-level `pub` items and simple-symbol `pub use`, excluding grouped and glob forms; Dart evidence covers URI-level module re-exports from a package root or its `lib` directory while `show` and `hide` filtering and broader monorepo-root package discovery remain unmeasured. Dart calls also meet the sampled floor, including exact `BookstoreAuth.of(context)` resolution and a same-name `GoRouter.of(context)` decoy; this does not establish general framework or monorepo call resolution. Kotlin calls meet the sampled floor for callsite-owner attribution, including the Now in Android constructor-owned line-45 `map` call and the line-57 wrong-caller decoy; the target remains unresolved, so general typed, framework, and monorepo call resolution remain unmeasured. Kotlin types meet the sampled floor for reviewed constructor and type-use resolution in Coroutines and Ktor. Generic constructor calls and property initializer constructions remain missed, `List(size) { ... }` can appear as unresolved construction, and full type inference, generic substitution, nullability flow, overload resolution, and compiler-equivalent analysis remain unmeasured. C++ types meet the sampled floor, with fmt evidence preserving the `utf8_system_category` class while rejecting macro-shaped `FMT_STRING(...)` as a construction edge; general preprocessor expansion, template analysis, and C++ type resolution remain unmeasured. Grouped PHP imports remain unsupported and unmeasured. Every Tier 1 language remains unmeasured overall; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
