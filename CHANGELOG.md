...
- Tier 1 documentation now names the 50 capability rows that meet the sampled Phase 2 floor and the 104 rows that remain unmeasured or not applicable. TypeScript export and type-inference evidence now meet the sampled floor across its fixture and all three pinned repositories; imports remain explicitly unmeasured. JavaScript remains the first language whose applicable sampled rows all meet the floor; the other thirteen remain unmeasured, the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
