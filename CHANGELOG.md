...
- Tier 1 documentation now names the 52 capability rows that meet the sampled Phase 2 floor and the 102 rows that remain unmeasured or not applicable. TypeScript exact-import, export, and type-inference evidence and Python exact-import evidence now meet the sampled floor across each language's fixture and all three pinned repositories. Python heritage, types, configuration, and frameworks remain unmeasured. TypeScript and JavaScript are still the only languages whose Phase 2 applicable sampled rows all meet the floor; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
