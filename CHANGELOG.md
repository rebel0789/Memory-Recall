...
- Tier 1 documentation now names the 68 capability rows that meet the sampled Phase 2 floor, the 85 applicable rows that remain unmeasured, the zero recorded floor failures, and the single not-applicable row. Go and Rust exports now meet the sampled fixture-plus-three-repository floor. Go evidence omits exported fields and interface methods; Rust exact evidence is limited to unrestricted top-level `pub` items and simple-symbol `pub use`, excluding grouped and glob forms. Grouped PHP imports remain unsupported and unmeasured. Every Tier 1 language remains unmeasured overall; the JS engine remains the public default, and no competitor, parity, leadership, multi-repository, or scale claim is made.
