...
Current evidence commit: `d83121c9d2b917a63ef1a4171788d57b22aa4e4f`
...
| V-2 | Do not treat the approximately 89,000-line expansion as an assumed patch. | Contradicted | `main...d83121c` changes 351 files with 95,260 insertions and 2,594 deletions. The published baseline is `1.1.0`, while the worktree remains labeled `1.1.1`. `MEMORY_RECALL_SEMVER_COMPATIBILITY_AUDIT.md` identifies breaking distribution, MCP-config, Recall Map wire, default-engine, and shipped Rust-source contracts. | Restore or dual-serve every break before considering a minor line; otherwise use a major stable line after freeze. Do not choose or change the version yet. |
...
| ID | Requirement | Status | Authoritative evidence | Required closure |
| --- | --- | --- | --- | --- |
| A-1 | Rust owns production code intelligence; Node owns transport, validation, memory, CLI, MCP, API, and UI. | Incomplete | ADR 0023 defines the boundary. CLI, MCP, Control API, and web now select the packaged Rust SQLite index by default and fail closed with exact recovery guidance. The frozen JS/TS implementation remains reachable only through explicit `compatibility` mode. | Prove the packaged native path on clean installs, then delete the duplicate JS intelligence implementation and duplicate tests. |
...
| A-3 | Use a persistent incremental embedded index; JSON is bounded export/debug output only. | Incomplete | Phase 3 proves SQLite generations and bounded incremental refresh over 781 files. Production-default CLI, MCP, API, and web reads now use the SQLite index without mutating it; explicit `compatibility` mode still exposes the temporary JS graph. | Prove the packaged native path, then delete the compatibility implementation so SQLite is the only intelligence store. |
...
| ID | Requirement | Status | Authoritative evidence | Required closure |
| --- | --- | --- | --- | --- |
| P-1 | Packaged Rust is the production default in CLI. | Incomplete | Commands without `--engine` now require the packaged native binary and current SQLite index; `auto` and `native-preview` are strict native aliases, while JS requires explicit `compatibility`. Focused CLI tests prove native reads do not mutate the index. | Prove clean first-run indexing and every CLI structural workflow from the packed product. |
| P-2 | Packaged Rust is the production default in MCP. | Incomplete | Default MCP and the strict aliases now use only a current native index and return actionable bounded errors instead of falling back. Focused tests prove current, stale, and between-call freshness behavior. | Prove all twelve tools from a clean packed install without selecting compatibility mode. |
| P-3 | Packaged Rust is the production default in web/Control API. | Incomplete | Control API and web now use the native index by default, expose bounded unavailable previews for missing/stale/query-failure states, and render exact recovery actions. | Prove the complete browser path from a clean packed install and all recovery states. |
