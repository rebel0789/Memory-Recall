...
Current evidence commit: `c02e986`
...
| ID | Requirement | Status | Authoritative evidence | Required closure |
| --- | --- | --- | --- | --- |
| U-1 | First screen explains shape, coverage, subsystems, entry points, hotspots, changes, memory, and next action. | Incomplete | `c02e986` runs Chromium against a clean 1,225-tracked-file clone of this repository. The first desktop screen shows six of twelve architectural groups in a readable grid, explicitly discloses the complete bounded outline, carries current coverage truth, and keeps all orientation regions inside the viewport. Not every item is proven on a representative large polyglot repository through the packaged native path. | Run task-based browser proof on representative large repositories and correct comprehension gaps. |
| U-2 | Map uses progressive disclosure for groups, communities, processes, neighborhoods, and evidence. | Incomplete | `c02e986` directly loads `/map?group=apps%2Fweb` against that clean clone and proves a bounded focused map, keyboard outline selection, safely truncated outline labels, graph canvas, evidence inspector, and entry-to-sink process disclosure. The visual board exposes six groups while the outline keeps all twelve. Broader density controls and evidence drill-down remain unproven. | Prove density controls and evidence drill-down on representative large graphs. |
| U-3 | Navigation is understandable and Map works directly. | Incomplete | First-run browser proof reaches Map from Overview and directly runs a submitted Map query. `c02e986` proves a direct large-repository `/map` request renders without a second click. The browser clicks every visible desktop and mobile primary-navigation destination and asserts its destination route and rendered content. Task-based comprehension on a representative large repository remains unproven. | Browser-test users' likely first actions on a large repository. |
| U-4 | No overlap or unreadable source-map outline. | Incomplete | `c02e986` retains desktop and 390px captures of a real large-repository focused map: canvas labels stay bounded, outline labels/locators use ellipsis, no desktop/mobile horizontal overflow occurs, and the final mobile outline item remains above navigation. No full visual audit across representative large repositories and viewports is recorded. | Capture desktop/mobile screenshots and test long labels, narrow inspectors, zoom, overflow, and selection on representative large repositories. |
| U-5 | Desktop, mobile, accessibility, overflow, and every control pass in a real browser. | Incomplete | Consumer browser smoke covers every published route at desktop width for meaningful content, overflow, one main landmark, one page heading, accessible names, and keyboard reachability on visible controls. It also covers direct Map, reload, graph controls, keyboard outline selection, console/page errors, reduced motion, mobile overflow, and mobile outline reachability. `c02e986` adds the same overflow and keyboard checks on a 1,225-file clone. It is not the complete native large-repository control inventory or an accessibility audit. | Exercise every business control at desktop/mobile widths and run an accessibility audit on a representative large repository. |
...
| ID | Requirement | Status | Authoritative evidence | Required closure |
| --- | --- | --- | --- | --- |
| R-1 | Full Node CI on frozen implementation. | Incomplete | Full Node CI passes at `c02e986` after the real large-repository Map proof and regenerated release-readiness artifacts; implementation is not frozen. | Run once more only after all required implementation is complete. |
