...
Current evidence commit: `2273cdc3c7b821de261d5c4d778032515a114e36`
...
| V-2 | Do not treat the approximately 89,000-line expansion as an assumed patch. | Contradicted | `main...2273cdc` changes 351 files with 94,956 insertions and 2,597 deletions. The published baseline is `1.1.0`, while the worktree remains labeled `1.1.1`. `MEMORY_RECALL_SEMVER_COMPATIBILITY_AUDIT.md` identifies breaking distribution, MCP-config, Recall Map wire, default-engine, and shipped Rust-source contracts. | Restore or dual-serve every break before considering a minor line; otherwise use a major stable line after freeze. Do not choose or change the version yet. |
...
Phase 2 currently reports 70 `meets-floor` rows, 83 applicable `unmeasured` rows, and one explicit `not-applicable` row. No row is recorded as `does-not-meet-floor`, which means missing proof must not be relabeled as support.
...
| L-7 | Calls meet at least 90% reviewed precision with confidence evidence. | Incomplete | Phase 2 sampled call precision passes where measured. Dart now includes fixture plus three-repository evidence, including a typed Flutter call and a same-name wrong-callsite decoy; Kotlin, Swift, and several other call/type combinations remain unmeasured or governed-memory-limited. | Add reviewed positive/negative calls and confidence strategies per remaining language without weakening identity safety. |
...
| L-13 | Results are deterministic and contain no duplicate canonical symbols. | Incomplete | Phase 2 sampled graphs are deterministic with zero recorded duplicates; five repository scopes report omissions and 83 applicable capability rows remain unmeasured. | Re-run determinism/duplicate checks for every completed capability and final packaged binaries. |
| L-14 | Partial, unsupported, and not-applicable behavior is explicit. | Incomplete | Tier 1 applicability is now independent of evidence presence. All applicable unsupported or missing rows remain non-green; C heritage is the sole explicit not-applicable row with a language-semantic rationale. Eighty-three applicable rows remain unmeasured. | Prove or explicitly fail every remaining applicable row without converting missing evidence into not-applicable support. |
...
| H-4 | Meet floors: symbol recall at least 95%, resolved-call precision at least 90%, zero duplicate symbols, determinism, explicit unsupported states, and real-repository proof. | Incomplete | Sampled Memory Recall rows meet floors where measured; 83 applicable rows and competitor runs are missing. | Close all applicability rows and execute the full comparison. |
...
| 2 | Pass fourteen Tier 1 languages. | Incomplete | 70/154 rows meet floor; 83 applicable rows remain unmeasured and one row is not applicable. All fourteen language-level statuses remain unmeasured. |
...
2. Close the 83 applicable language rows without weakening gates.
