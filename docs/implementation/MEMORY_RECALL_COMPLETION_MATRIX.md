...
Current evidence commit: `3517843745fdd2981831cced3ab390d10088b1a9`
...
| D-2 | No root package may reference unpublished native packages. | Incomplete | Root `optionalDependencies` still references five `@memory-recall/native-*@1.1.1` packages that currently return npm `E404`. The protected workflow now validates the exact root artifact, publishes and verifies the five native packages in fixed order, and only then permits the exact root tarball dry run and publication. | Prove the guard in the remote protected workflow and publish no root until all five native packages pass it. |
...
Phase 2 currently reports 61 `meets-floor` rows, 92 applicable `unmeasured` rows, and one explicit `not-applicable` row. No row is recorded as `does-not-meet-floor`, which means missing proof must not be relabeled as support.
...
| L-3 | Packages, modules, imports, exports, and bindings resolve exactly. | Incomplete | TypeScript, JavaScript, Python, Java, Go, Rust, Kotlin, and C# now have selected import evidence. Java regular and static imports preserve their full targets; Go preserves external coordinates and canonicalizes internal module targets; Rust, Kotlin, and C# preserve full external coordinates across each fixture and three pinned repositories. Remaining import/export rows stay unmeasured. | Add fixture plus three-repository truth for every applicable package/import/export/binding row. |
...
| L-7 | Calls meet at least 90% reviewed precision with confidence evidence. | Incomplete | Phase 2 sampled call precision passes where measured; Kotlin, Swift, Dart and several other call/type combinations remain unmeasured or governed-memory-limited. | Add reviewed positive/negative calls and confidence strategies per language without weakening identity safety. |
...
| L-13 | Results are deterministic and contain no duplicate canonical symbols. | Incomplete | Phase 2 sampled graphs are deterministic with zero recorded duplicates; five repository scopes report omissions and 92 applicable capability rows remain unmeasured. | Re-run determinism/duplicate checks for every completed capability and final packaged binaries. |
| L-14 | Partial, unsupported, and not-applicable behavior is explicit. | Incomplete | Tier 1 applicability is now independent of evidence presence. All applicable unsupported or missing rows remain non-green; C heritage is the sole explicit not-applicable row with a language-semantic rationale. Ninety-two applicable rows remain unmeasured. | Prove or explicitly fail every remaining applicable row without converting missing evidence into not-applicable support. |
...
| Kotlin | 8 | exports, heritage, types, calls, config, frameworks, impact, processes |
| C# | 7 | exports, heritage, types, config, frameworks, impact, processes |
| Go | 7 | exports, heritage, types, config, frameworks, impact, processes |
| Rust | 6 | exports, heritage, config, frameworks, impact, processes |
| PHP | 7 | imports, exports, types, config, frameworks, impact, processes |
| Ruby | 8 | imports, exports, heritage, types, config, frameworks, impact, processes |
| Swift | 7 | imports, exports, calls, config, frameworks, impact, processes |
| C | 7 | imports, exports, types, config, frameworks, impact, processes |
| C++ | 8 | imports, exports, heritage, types, config, frameworks, impact, processes |
| Dart | 7 | exports, heritage, calls, config, frameworks, impact, processes |

## Phase 4 intelligence

| ID | Requirement | Status | Authoritative evidence | Required closure |
| --- | --- | --- | --- | --- |
| I-1 | Deterministic communities on real repositories. | Incomplete | The Phase 4 receipt regenerated from `7f4f34a` runs `label-propagation-v1` five times on exact pinned Express, Nest cats-sample, and Gin scopes (260 files, 4,564 nodes, 13,640 edges combined), records deterministic results, and proves two non-overlapping cursor pages for every repository. It does not contain reviewed community truth or large-repository stability evidence. | Add reviewed community truth and stability metrics on representative pinned repositories and large repos. |
...
| H-4 | Meet floors: symbol recall at least 95%, resolved-call precision at least 90%, zero duplicate symbols, determinism, explicit unsupported states, and real-repository proof. | Incomplete | Sampled Memory Recall rows meet floors where measured; 92 applicable rows and competitor runs are missing. | Close all applicability rows and execute the full comparison. |
...
| 2 | Pass fourteen Tier 1 languages. | Incomplete | 61/154 rows meet floor; 92 applicable rows remain unmeasured and one row is not applicable. All fourteen language-level statuses remain unmeasured. |
...
| R-2 | Full Rust workspace on frozen implementation. | Incomplete | The current evidence slice passed the full Rust workspace: 113 unit and integration tests plus all doc tests; implementation is not frozen. | Run at final freeze. |
...
2. Close the 92 applicable language rows without weakening gates.
