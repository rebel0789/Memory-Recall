...
| T-2 | Meet or beat GitNexus across the fourteen Tier 1 languages. | Incomplete | Phase 2 has 14 fixtures and 43 repositories, but only 56 of 154 capability rows meet the sampled floor; 98 are unmeasured. | Resolve every applicable row with fixture and capability-specific evidence from at least three pinned repositories per language. |
...
| D-6 | Produce complete native provenance and SBOM. | Incomplete | The local workflow produces one deterministic SPDX 2.3 document per target with the exact five package files, SHA-1/SHA-256 checksums, package verification code, tarball checksum, version-qualified namespace, and signed SBOM predicate. No remote five-target attestation exists. | Obtain and verify all provenance and SBOM attestations from one successful frozen-commit run. |
...
| D-9 | Publish checksums, provenance, and SBOM with the stable release. | Incomplete | The local protected workflow consumes one exact five-target artifact, verifies signed GitHub provenance and SPDX predicates, and requires both npm publish and SLSA attestations for every native package. The root package evidence path and an authorized stable release are still absent. | Bind the exact root tarball and its npm provenance, then attach the frozen evidence set only during the authorized stable release. |

## Fourteen-language support gate

Phase 2 currently reports 56 `meets-floor` rows and 98 `unmeasured` rows. No row is recorded as `does-not-meet-floor`, which means missing proof must not be relabeled as support.
...
| L-3 | Packages, modules, imports, exports, and bindings resolve exactly. | Incomplete | TS/JS/Python have selected import/export evidence; most other languages have unmeasured import/export rows. | Add fixture plus three-repository truth for every applicable package/import/export/binding row. |
...
| Java | 8 | imports, exports, heritage, types, config, frameworks, impact, processes |
...
| H-4 | Meet floors: symbol recall at least 95%, resolved-call precision at least 90%, zero duplicate symbols, determinism, explicit unsupported states, and real-repository proof. | Incomplete | Sampled Memory Recall rows meet floors where measured; 98 rows and competitor runs are missing. | Close all applicability rows and execute the full comparison. |
...
| 6 | Signed npm distribution. | Incomplete | The local producer/consumer chain is native-first and verifies signed GitHub provenance, file-complete SPDX predicates, npm integrity, and native npm attestation bundles; root-package evidence, remote five-platform proof, and publication remain absent. |
...
1. Bind and verify the exact root tarball, then remote-prove D-1 and D-2 from one frozen commit.
