...
Current evidence commit: `62d7c3a3fd7340c2e159789c0be14b3b72d96900`
...
| V-2 | Do not treat the approximately 89,000-line expansion as an assumed patch. | Contradicted | `main...62d7c3a` changes 351 files with 95,164 insertions and 2,596 deletions. The published baseline is `1.1.0`, while the worktree remains labeled `1.1.1`. `MEMORY_RECALL_SEMVER_COMPATIBILITY_AUDIT.md` identifies breaking distribution, MCP-config, Recall Map wire, default-engine, and shipped Rust-source contracts. | Restore or dual-serve every break before considering a minor line; otherwise use a major stable line after freeze. Do not choose or change the version yet. |
...
Phase 2 currently reports 74 `meets-floor` rows, 79 applicable `unmeasured` rows, and one explicit `not-applicable` row. No row is recorded as `does-not-meet-floor`, which means missing proof must not be relabeled as support.
...
| L-5 | Heritage, interfaces, traits, and protocols resolve. | Incomplete | Python, Dart, and selected languages have sampled heritage. Dart now has fixture plus three-repository evidence for a direct generic superclass, mixin, interface, and extension-on-type edge, including a negative generic-argument decoy; most heritage rows remain unmeasured. | Add reviewed heritage truth for every remaining applicable language and explicit not-applicable decisions where the language lacks the construct. |
...
| L-13 | Results are deterministic and contain no duplicate canonical symbols. | Incomplete | Phase 2 sampled graphs are deterministic with zero recorded duplicates; five repository scopes report omissions and 79 applicable capability rows remain unmeasured. | Re-run determinism/duplicate checks for every completed capability and final packaged binaries. |
| L-14 | Partial, unsupported, and not-applicable behavior is explicit. | Incomplete | Tier 1 applicability is now independent of evidence presence. All applicable unsupported or missing rows remain non-green; C heritage is the sole explicit not-applicable row with a language-semantic rationale. Seventy-nine applicable rows remain unmeasured. | Prove or explicitly fail every remaining applicable row without converting missing evidence into not-applicable support. |
...
| C++ | 6 | exports, heritage, config, frameworks, impact, processes |
...
| 2 | Pass fourteen Tier 1 languages. | Incomplete | 74/154 rows meet floor; 79 applicable rows remain unmeasured and one row is not applicable. All fourteen language-level statuses remain unmeasured. |
...
2. Close the 79 applicable language rows without weakening gates.
