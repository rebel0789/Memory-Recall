...
| Local Rust persistent index preview | Build locally, set `MEMORY_RECALL_NATIVE_BINARY`, then use `graph index` with `--engine native-preview`; writer modes remain explicit | Writes only `.local/source-index/index.v1.sqlite` for explicit build, refresh, or confirm-gated repair | None | SQLite generations, incremental refresh, doctor, repair, bounded query, and exact no-op refresh evidence across 14 reviewed fixtures | Experimental |
