...
import { analyzeContentPatterns, assessContentObservation, completeLocalCreatorWorkflow, observationRecordKind } from '../../packages/content-intelligence/src/index.mjs';
...
export async function runContentIntelligence({emit=async()=>{},env=process.env,objective='Find evidence-backed content angles about reliable local agents',workspaceId='ws_local',actorId='agent:researcher',manifestRepository=null,sources=null,telemetry=null,traceContext=null,correlationId=null}={}) {
  ...
  const steps=[
    ...
    {id:'local-draft-outcome',kind:'deterministic',timeoutMs:5000,retry:{maxAttempts:1},actorId:'agent:orchestrator',run:async({outputs})=>{const candidate=outputs['generate-angles'].output[0];return completeLocalCreatorWorkflow({workspaceId,actorId,candidate,objective,availableEvidenceIds:outputs['verify-recommendations'].availableEvidenceIds,contextManifest:outputs['compile-context'],modelResult:outputs['generate-angles'],sourceRecords:outputs.normalize,patternAnalysis:outputs['analyze-patterns'],editedText:[candidate.angle,'Show the retry boundary, the denied action, and the evidence IDs before asking anyone to trust the agent.'].join('\n\n'),metric:{name:'creator_judgment',value:'accepted_for_local_review',direction:'positive'},now:'2026-06-19T10:00:00.000Z',expiresAt:'2026-06-19T10:15:00.000Z'})},summarize:result=>({approvalStatus:result.approval.status,draftId:result.draft.id,draftFingerprint:result.draft.draftFingerprint,verified:result.verification.valid,copyingRisk:result.verification.similarity.copyingRisk.level,outcomeMetric:result.outcome.objectiveMetric.name,publisherEnabled:result.draft.publisher.enabled})}
  ];
  const result=await executeSteps({runId,workspaceId,steps,emit,telemetry,traceContext,correlationId,workflowId:'workflow:content-intelligence',workflowVersion:'0.1.0'});
