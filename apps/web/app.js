...
function contextPackLaunchPath(model,readiness) {
  ...
  const memoryControls=model.memoryConfig.configured
    ? `<div class="action-row"><button class="button secondary" data-action="copy-memory-config" type="button">Copy memory config</button><button class="button secondary" data-action="download-memory-config" type="button">Download memory config</button></div><small>${Number(model.memoryConfig.pathCount)} reviewed file${model.memoryConfig.pathCount===1?'':'s'}; place ${esc(model.memoryConfig.downloadName)} at the repo root before running preflight.</small>`
