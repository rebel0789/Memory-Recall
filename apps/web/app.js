...
function contextPackCommandList(commands) {
  return `<ol class="command-list">${commands.map((item)=>`<li><div class="command-heading"><strong>${esc(item.label)}</strong><button class="button secondary command-copy" data-action="copy-command" type="button" aria-label="Copy ${esc(item.label)} command">Copy</button></div><code>${esc(item.command)}</code></li>`).join('')}</ol>`;
}
...
function contextPackOperatorBrief(model) {
  ...
  const commands=model.commands
    .filter((item)=>['Test local handoff','Copy impact command','Start MCP bridge'].includes(item.label))
    .slice(0,3);
  ...
  return `<section class="handoff-brief" aria-label="Handoff operator brief"><div class="handoff-brief-head"><div><p class="eyebrow">Use this pack</p><h2>Changed files, reads, and proof commands are ready.</h2></div><span>${esc(model.targetHarness)} · ${esc(model.fingerprintShort)}</span></div><div class="handoff-brief-grid"><article><h3>Changed</h3><dl class="facts compact-facts"><div><dt>Coverage</dt><dd>${esc(brief.changedCoverageLabel)} (${esc(brief.changedCoveragePercent)})</dd></div><div><dt>Symbols</dt><dd>${Number(brief.affectedSymbolCount)} affected</dd></div><div><dt>Top impact</dt><dd>${symbols}</dd></div><div><dt>Hash proof</dt><dd>${Number(brief.changedHashVerifiedCount)} changed files</dd></div></dl></article><article><h3>Read first</h3>${reads}<p class="muted">Raw source bodies, markdown bodies, local paths, model calls, network calls, and adapters stay out of this brief.</p></article><article><h3>Run proof</h3>${commandList}</article></div></section>`;
}
