...
test('Phase 4 intelligence evidence proves deterministic bounded projections without parity claims', async () => {
  ...
  assert.equal(report.results.representativeSearch.exactLabel, 'handleUser');
  assert.equal(report.results.representativeSearch.lexicalLabel, 'handleUserUtility');
  assert(report.results.representativeSearch.neighborNodeId);
  ...
  assert.deepEqual(
    representative.executionSteps.map((item) => [item.fromNodeId, item.toNodeId]),
    representative.nodeIds.slice(0, -1).map((nodeId, index) => [nodeId, representative.nodeIds[index + 1]])
  );
  assert.equal(report.claims.competitorParity, false);
  assert.equal(report.claims.leadership, false);
  assert.equal(report.claims.millionNodeScale, false);
