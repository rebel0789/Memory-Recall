...
test('CLI topic help treats valid help requests as read-only success',()=>{for(const [args,patterns] of [
  ...
  [['mcp','--help'],[/Memory Recall CLI: mcp/,/oaf mcp inspect --read-only --root \. --format json/,/oaf:\/\/workspace\/ws_local\/skills\/catalog/,/oaf mcp server --read-only --root \. --stdio/,/Direct MCP server commands and mcp install default to auto mode/,/oaf graph index --write --engine native-preview --root \. --format summary/,/Resource\/server paths require/]]
