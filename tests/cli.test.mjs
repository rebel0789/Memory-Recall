...
test('measure context-pack all-shards JSON stays on the measurement schema',()=>{
  ...
  const gitAddBaseline=spawnSync('git',['add','package.json','AGENTS.md'],{cwd:root,encoding:'utf8'});
  ...
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','measure','context-pack','--read-only','--root',root,'--from','codex','--objective','Measure every changed shard safely','--step','aggregate all shards','--target','codex','--changed-from-git','--all-shards','--format','json'],{encoding:'utf8',env});
  assert.equal(result.status,0,result.stderr);
  const report=JSON.parse(result.stdout);
  ...
  assert.equal(report.summary.changedLocatorShardCount,2);
  assert.equal(report.summary.shardsMeasured,2);
  assert.equal(report.summary.totalChangedLocatorCount,17);
  assert.equal(report.summary.measuredChangedLocatorCount,17);
  ...
  assert.deepEqual(report.shards.map((shard)=>shard.measuredChangedLocatorCount),[16,1]);
