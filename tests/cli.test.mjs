...
test('CLI topic help treats valid help requests as read-only success',()=>{for(const [args,patterns] of [
  ...
  [['mcp','--help'],[/Memory Recall CLI: mcp/,/oaf mcp inspect --read-only --root \. --format json/,/oaf:\/\/workspace\/ws_local\/skills\/catalog/,/oaf mcp server --read-only --root \. --stdio/,/Direct MCP server commands without --engine keep the JS default/,/mcp install\s+configures auto mode and prints indexBuildCommand/,/oaf graph index --write --engine native-preview --root \. --format summary/,/Resource\/server paths require/]]
...
test('graph stats reports read-only graph quality and token evidence without search payloads',()=>{
  ...
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','stats','--root',root,'--format','json'],{encoding:'utf8',env});
  assert.equal(result.status,0,result.stderr);
  const report=JSON.parse(result.stdout);
  ...
  const summary=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','stats','--root',root,'--format','summary'],{encoding:'utf8',env});
  assert.equal(summary.status,0,summary.stderr);
