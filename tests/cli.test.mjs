...
test('CLI topic help treats valid help requests as read-only success',()=>{for(const [args,patterns] of [
  ...
  [['help','graph'],[/Open Agent Fabric CLI: graph/,/oaf graph stats --root \./,/oaf graph search --root \. --query/,/oaf graph trace --root \. --symbol/,/oaf graph impact --root \. --changed/,/raw source bodies are not included/]],
