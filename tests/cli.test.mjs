...
function buildNativeIndex(root, env=process.env){const result=spawnSync(process.execPath,[CLI_PATH,'graph','index','--write','--engine','native','--root',root,'--format','json'],{encoding:'utf8',env});assert.equal(result.status,0,result.stderr);}
...
test('context handoff read-only report proves Codex-ready MCP bridge without writes',()=>{
  ...
  const env={...process.env,OAF_FIXED_NOW:'2026-06-24T00:00:00.000Z',OAF_COMMIT_SHA:'1234567890abcdef1234567890abcdef12345678'};buildNativeIndex(root,env);
...
test('graph stats reports read-only graph quality and token evidence without search payloads',()=>{
  ...
  const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};buildNativeIndex(root,env);
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','stats','--engine','native','--root',root,'--format','json'],{encoding:'utf8',env});
  assert.equal(result.status,0,result.stderr);
  const report=JSON.parse(result.stdout);
  ...
  assert.equal('qualifiedSymbolCount' in report.stats,false);
  assert.equal('ambiguousSymbolLabelCount' in report.stats,false);
  ...
  const summary=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','stats','--engine','native','--root',root,'--format','summary'],{encoding:'utf8',env});
  assert.equal(summary.status,0,summary.stderr);
  assert.match(summary.stdout,/# Graph Stats/);
  assert.match(summary.stdout,/Qualified symbols: unmeasured/);
  assert.match(summary.stdout,/Ambiguous labels: unmeasured/);
  assert.match(summary.stdout,/Hotspots/);
  assert.match(summary.stdout,/workspace:\/\/src\/auth\.ts/);
...
test('MCP code.impact returns bounded locator-safe impact and rejects unsafe map inputs',()=>{
  ...
  const env={...process.env,OAF_FIXED_NOW:'2026-07-11T12:00:00.000Z'};buildNativeIndex(root,env);
  const input=[
    JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{}}),
    JSON.stringify({jsonrpc:'2.0',id:2,method:'tools/list'}),
  ...
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','mcp','server','--read-only','--engine','native','--root',root,'--sqlite','.local/memory.sqlite','--stdio'],{encoding:'utf8',env,input});
  assert.equal(result.status,0,result.stderr);
  const responses=result.stdout.trim().split(/\n/u).map((line)=>JSON.parse(line));
  ...
  assert.equal(impact.data.results.length,1);
  assert.equal(impact.data.results[0].nodes.length<=1,true);
