...
test('CLI topic help treats valid help requests as read-only success',()=>{for(const [args,patterns] of [
  [['help','context'],[/Memory Recall CLI: context/,/oaf handoff/,/--include-file notes\/handoff\.md/,/oaf context handoff --read-only/,/oaf context graph preview --root \. --query/,/Graph preview is dry-run only/]],
  [['help','graph'],[/Memory Recall CLI: graph/,/oaf graph stats --root \./,/oaf graph search --root \. --query/,/oaf graph trace --root \. --symbol/,/oaf graph impact --root \. --changed/,/raw source bodies are not included/]],
  [['handoff','--help'],[/Memory Recall CLI: handoff/,/With no flags it defaults to a compact/,/does not\s+write files/]],
  [['token-saver','--help'],[/Memory Recall CLI: token-saver/,/With no flags it measures/,/provider billing-token savings/]],
  [['context','handoff','--help'],[/Memory Recall CLI: context handoff/,/zero-tool MCP proof/,/does not write files/]],
  [['context','receive','--help'],[/Memory Recall CLI: context receive/,/versioned receiver packet/,/rejects task text/]],
  [['context','registry','status','--help'],[/Memory Recall CLI: context registry/,/verifies the current pointer/,/does not rebuild/]],
  [['setup','--help'],[/Memory Recall CLI: setup/,/Creates only local Recall state/,/does not\s+scan\s+source files/,/oaf map --root \. --sqlite \.local\/memory\.sqlite --format summary/]],
  [['connect','--help'],[/Memory Recall CLI: connect/,/dry-run by default/,/writes only the local\s+harness client config/]],
  [['disconnect','--help'],[/Memory Recall CLI: disconnect/,/dry-run by default/,/removes only\s+matching local harness config entries/]],
  [['harness','setup','--help'],[/Memory Recall CLI: harness setup/,/preview\s+only/,/does not mutate home config/]],
  [['memory','refine','--help'],[/Memory Recall CLI: memory refine/,/lineage\s+residue/,/--target-active-facts 200/,/does\s+not create active memory/,/state: "unavailable"/]],
  [['skill','catalog','--help'],[/Memory Recall CLI: skill catalog/,/does not include raw skill text/,/grant tool\s+authority/]],
  [['skill','load-plan','--help'],[/Memory Recall CLI: skill load-plan/,/ordered local reads/,/does not include raw skill text/]],
  [['measure','context-pack','--help'],[/Memory Recall CLI: measure context-pack/,/changed-file coverage/,/does not include raw source bodies/]],
  [['mcp','--help'],[/Memory Recall CLI: mcp/,/oaf mcp inspect --read-only --root \. --format json/,/oaf:\/\/workspace\/ws_local\/skills\/catalog/,/oaf mcp server --read-only --root \. --stdio/,/Direct MCP server commands and mcp install default to native mode/,/oaf graph index --write --engine native --root \. --format summary/,/Resource\/server paths require/]]
...
test('graph search trace and impact commands expose locator-only repo intelligence',()=>{const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-commands-'));mkdirSync(path.join(root,'src'),{recursive:true});writeFileSync(path.join(root,'src','auth.ts'),['function add(left, right) {','  return left + right;','}','','export function approveTokenReset() {',"  return add(1, 1) === 2 && 'CLI GRAPH COMMAND RAW BODY';",'}'].join('\n'));writeFileSync(path.join(root,'src','workflow.ts'),["import { approveTokenReset } from './auth';",'export function runAuthWorkflow() {','  return approveTokenReset();','}'].join('\n'));const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};const search=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','search','--engine','compatibility','--root',root,'--query','approve token reset','--format','json'],{encoding:'utf8',env});assert.equal(search.status,0,search.stderr);const searchReport=JSON.parse(search.stdout);assert.equal(searchReport.command,'graph search');assert(searchReport.search.results.some(item=>item.label==='approveTokenReset'&&item.symbolKind==='function'));assert.equal(searchReport.safeguards.rawBodyIncluded,false);assert(searchReport.measurements.fullGraphTokenEstimate>0);assert(searchReport.measurements.deliveredTokenEstimate>0);assert.equal(searchReport.measurements.providerBillingClaimed,false);const symbolSearch=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','search','--engine','compatibility','--root',root,'--query','approve token reset','--node-kinds','symbol','--format','json'],{encoding:'utf8',env});assert.equal(symbolSearch.status,0,symbolSearch.stderr);const symbolReport=JSON.parse(symbolSearch.stdout);assert(symbolReport.search.results.length>0);assert(symbolReport.search.results.every(item=>item.resultType==='node'&&item.kind==='symbol'&&item.symbolKind));const callSearch=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','search','--engine','compatibility','--root',root,'--query','approve token reset','--edge-kinds','calls','--format','json'],{encoding:'utf8',env});assert.equal(callSearch.status,0,callSearch.stderr);const callReport=JSON.parse(callSearch.stdout);assert(callReport.search.results.length>0);assert(callReport.search.results.every(item=>item.resultType==='edge'&&item.kind==='calls'));const trace=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','trace','--engine','compatibility','--root',root,'--symbol','runAuthWorkflow','--format','summary'],{encoding:'utf8',env});assert.equal(trace.status,0,trace.stderr);assert.match(trace.stdout,/# Graph Trace/);assert.match(trace.stdout,/approveTokenReset/);assert.match(trace.stdout,/Delivered graph tokens:/);const impact=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','impact','--engine','compatibility','--root',root,'--changed','src/auth.ts','--format','summary'],{encoding:'utf8',env});assert.equal(impact.status,0,impact.stderr);assert.match(impact.stdout,/# Graph Impact/);assert.match(impact.stdout,/Changed coverage: 1\/1/);assert.match(impact.stdout,/Impacted edges: [1-9]\d*/);assert.match(impact.stdout,/Impact edge kinds: .*calls [1-9]\d*/);assert.match(impact.stdout,/approveTokenReset/);assert(impact.stdout.indexOf('approveTokenReset')<impact.stdout.indexOf('add (function)'),impact.stdout);assert.match(impact.stdout,/Full graph JSON tokens avoided:/);for(const output of [search.stdout,symbolSearch.stdout,callSearch.stdout,trace.stdout,impact.stdout]){assert.doesNotMatch(output,/CLI GRAPH COMMAND RAW BODY/);assert.doesNotMatch(output,new RegExp(root.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));assert.doesNotMatch(output,/\/Users\//);}});
test('graph search JSON stays compact and omits sampled graph payloads',()=>{const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-compact-'));mkdirSync(path.join(root,'src'),{recursive:true});writeFileSync(path.join(root,'src','auth.ts'),['export function approveTokenReset() {','  return true;','}'].join('\n'));const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','search','--engine','compatibility','--root',root,'--query','approve token reset','--format','json'],{encoding:'utf8',env});assert.equal(result.status,0,result.stderr);const report=JSON.parse(result.stdout);assert.equal(report.command,'graph search');assert.equal(report.graph.summary.fileCount,1);assert.equal(report.graph.diagnosticCount,0);assert.equal('sampleNodes' in report.graph,false);assert.equal('sampleEdges' in report.graph,false);assert.equal('diagnostics' in report.graph,false);assert(report.search.results.some(item=>item.label==='approveTokenReset'));});
test('graph stats reports read-only graph quality and token evidence without search payloads',()=>{
  const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-stats-'));
  mkdirSync(path.join(root,'src'),{recursive:true});
  writeFileSync(path.join(root,'src','auth.ts'),[
    ...
    '  return new AuthController().approveTokenReset();',
    '}'
  ].join('\n'));
  const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','stats','--engine','compatibility','--root',root,'--format','json'],{encoding:'utf8',env});
  assert.equal(result.status,0,result.stderr);
  const report=JSON.parse(result.stdout);
  assert.equal(report.command,'graph stats');
  assert.equal(report.graph.summary.fileCount,1);
  assert.equal(report.stats.fileCount,1);
  assert.equal(report.stats.qualifiedSymbolCount,2);
  assert.equal(report.stats.ambiguousSymbolLabelCount,1);
  assert.deepEqual(report.stats.ambiguousLabels.map((item)=>({label:item.label,count:item.count})),[{label:'approveTokenReset',count:2}]);
  assert.deepEqual([...report.stats.ambiguousLabels[0].qualifiedLabels].sort(),['AdminController.approveTokenReset','AuthController.approveTokenReset']);
  assert.equal(report.safeguards.rawBodyIncluded,false);
  assert.equal(report.measurements.providerBillingClaimed,false);
  assert(report.measurements.deliveredTokenEstimate>0);
  assert.equal('search' in report,false);
  assert.equal('trace' in report,false);
  assert.equal('impact' in report,false);
  const summary=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','stats','--engine','compatibility','--root',root,'--format','summary'],{encoding:'utf8',env});
  assert.equal(summary.status,0,summary.stderr);
  assert.match(summary.stdout,/# Graph Stats/);
  assert.match(summary.stdout,/Qualified symbols: 2/);
  assert.match(summary.stdout,/Ambiguous labels: 1/);
  assert.match(summary.stdout,/Ambiguous label samples/);
  assert.match(summary.stdout,/- approveTokenReset: 2 definitions \(AuthController\.approveTokenReset, AdminController\.approveTokenReset\)/);
  assert.match(summary.stdout,/Hotspots/);
  assert.match(summary.stdout,/- AuthController\.approveTokenReset .*workspace:\/\/src\/auth\.ts#L2-L4/);
  assert.match(summary.stdout,/Entry points/);
  assert.match(summary.stdout,/- runAuthWorkflow .*workspace:\/\/src\/auth\.ts#L11-L13/);
  assert.match(summary.stdout,/Provider billing claimed: no/);
  for(const output of [result.stdout,summary.stdout]){
    assert.doesNotMatch(output,new RegExp(root.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
    assert.doesNotMatch(output,/\/Users\//);
  }
});
test('graph search summary prints prototype scope for ambiguous symbols',()=>{const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-scope-'));mkdirSync(path.join(root,'lib'),{recursive:true});writeFileSync(path.join(root,'lib','reply.js'),['function Reply() {}','','Reply.prototype.code = function code(statusCode) {','  this.statusCode = statusCode;','  return this;','}'].join('\n'));const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','search','--engine','compatibility','--root',root,'--query','reply prototype code','--node-kinds','symbol','--format','summary'],{encoding:'utf8',env});assert.equal(result.status,0,result.stderr);assert(result.stdout.includes('Qualified symbols: 1'));assert(result.stdout.includes('Ambiguous labels: 0'));assert(result.stdout.includes('Reply.prototype.code (function) workspace://lib/reply.js#L3-L6'));assert.doesNotMatch(result.stdout,new RegExp(root.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));});
test('graph trace summary prints terminal locator and prototype scope',()=>{const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-trace-scope-'));mkdirSync(path.join(root,'lib'),{recursive:true});writeFileSync(path.join(root,'lib','reply.js'),['function Reply() {}','','Reply.prototype.code = function code(statusCode) {','  this.statusCode = statusCode;','  return this;','}','','export function runReply(reply) {','  return reply.code(200);','}'].join('\n'));const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','trace','--engine','compatibility','--root',root,'--symbol','runReply','--format','summary'],{encoding:'utf8',env});assert.equal(result.status,0,result.stderr);assert(result.stdout.includes('Reply.prototype.code workspace://lib/reply.js#L3-L6'));assert.doesNotMatch(result.stdout,new RegExp(root.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));});
test('graph trace locator prefix filters inbound callers',()=>{const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-trace-prefix-'));mkdirSync(path.join(root,'lib'),{recursive:true});mkdirSync(path.join(root,'test'),{recursive:true});writeFileSync(path.join(root,'lib','reply.js'),['export function send(reply) {','  return reply','}'].join('\n'));writeFileSync(path.join(root,'lib','route.js'),["import { send } from './reply.js'",'export function routeHandler(reply) {','  return send(reply)','}'].join('\n'));writeFileSync(path.join(root,'test','reply.test.js'),["import { send } from '../lib/reply.js'",'export function testCaller(reply) {','  return send(reply)','}'].join('\n'));const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','trace','--engine','compatibility','--root',root,'--symbol','send','--direction','inbound','--locator-prefix','workspace://lib/','--format','summary'],{encoding:'utf8',env});assert.equal(result.status,0,result.stderr);assert.match(result.stdout,/routeHandler workspace:\/\/lib\/route\.js/);assert.doesNotMatch(result.stdout,/testCaller/);assert.doesNotMatch(result.stdout,/workspace:\/\/test\//);});
test('graph export alias summary prints the exported symbol locator',()=>{const root=mkdtempSync(path.join(os.tmpdir(),'oaf-cli-graph-export-alias-'));mkdirSync(path.join(root,'src'),{recursive:true});writeFileSync(path.join(root,'src','local-alias.ts'),['function helper() {',"  return 'helper';",'}','','class Hono {','  request() { return true; }','}','','export { Hono as HonoBase };'].join('\n'));const env={...process.env,OAF_FIXED_NOW:'2026-06-23T00:00:00.000Z'};const search=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','search','--engine','compatibility','--root',root,'--query','HonoBase export','--edge-kinds','exports','--format','summary'],{encoding:'utf8',env});assert.equal(search.status,0,search.stderr);assert.match(search.stdout,/HonoBase -> Hono \(class\) workspace:\/\/src\/local-alias\.ts#L5-L7/);assert.doesNotMatch(search.stdout,/helper/);const trace=spawnSync(process.execPath,['apps/cli/oaf.mjs','graph','trace','--engine','compatibility','--root',root,'--symbol','HonoBase','--edge-kinds','exports','--format','summary'],{encoding:'utf8',env});assert.equal(trace.status,0,trace.stderr);assert.match(trace.stdout,/Start nodes: 1/);assert.match(trace.stdout,/Hono workspace:\/\/src\/local-alias\.ts#L5-L7/);});
...
test('bench locomo scores model-free retrieval coverage without transcript dumps',()=>{
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','bench','locomo','--read-only','--root','.','--dataset','evals/locomo/smoke.v1.json','--budget','512','--limit','4','--format','json'],{encoding:'utf8'});
  assert.equal(result.status,0,result.stderr);
  const report=JSON.parse(result.stdout);
  assert.equal(report.command,'bench locomo');
  ...
  assert.equal(report.safeguards.networkCalls,0);
  assert.equal(report.safeguards.modelCalls,0);
  assert.equal(report.safeguards.rawConversationBodiesIncluded,false);
  assert.equal(report.antiGaming.goldAnswerNotUsedForRetrieval,true);
  assert(!result.stdout.includes('/Users/rebel'));
  assert(!result.stdout.includes('I joined the robotics club yesterday'));
});
...
test('mcp server exposes governed memory recall and compressed profile over stdio', async (t) => {
  ...
  assert.deepEqual(lines[1].result.tools.map((tool) => tool.name).sort(), ['code.context', 'code.dependencies', 'code.impact', 'code.routes', 'code.search', 'code.trace', 'context.pack', 'context.profile', 'memory.recall', 'repo.architecture', 'repo.index_status', 'repo.map']);
...
test('mcp install dry-run prints exact token-saver config for coding clients without writes', () => {
  ...
  const expectedArgs = [path.join(root, 'apps', 'cli', 'oaf.mjs'), 'mcp', 'server', '--read-only', '--engine', 'native', '--root', root, '--sqlite', sqlitePath, '--stats', statsPath, '--stdio'];
  for (const client of ['claude-code', 'cursor', 'codex']) {
    ...
    assert.equal(report.indexBuildCommand, `recall graph index --write --engine native --root ${JSON.stringify(root)} --format summary`);
...
test('mcp install apply requires matching confirmation before writing config', () => {
  ...
  const expectedArgs = [cliPath, 'mcp', 'server', '--read-only', '--engine', 'native', '--root', realRoot, '--sqlite', path.join(realRoot, '.local', 'memory.sqlite'), '--stats', path.join(realRoot, '.local', 'mcp-stats.jsonl'), '--stdio'];
  const expectedIndexBuildCommand = `recall graph index --write --engine native --root ${JSON.stringify(realRoot)} --format summary`;
...
test('mcp install upgrades and uninstall recognizes the former owned native alias', () => {
  const home = mkdtempSync(path.join(os.tmpdir(), 'oaf-cli-mcp-legacy-native-alias-'));
  const configPath = path.join(home, '.cursor', 'mcp.json');
  const root = realpathSync(path.resolve('.'));
  mkdirSync(path.dirname(configPath), { recursive: true });
  const legacyArgs = [path.resolve('apps/cli/oaf.mjs'), 'mcp', 'server', '--read-only', '--engine', 'auto', '--root', root, '--sqlite', path.join(root, '.local', 'memory.sqlite'), '--stats', path.join(root, '.local', 'mcp-stats.jsonl'), '--stdio'];
  const writeLegacy = () => writeFileSync(configPath, `${JSON.stringify({ mcpServers: { oaf: { command: process.execPath, args: legacyArgs } } }, null, 2)}\n`);
  writeLegacy();

  const preview = spawnSync(process.execPath, ['apps/cli/oaf.mjs', 'mcp', 'install', '--client', 'cursor', '--home', home, '--root', root, '--format', 'json'], { encoding: 'utf8' });
  assert.equal(preview.status, 0, preview.stderr);
  const report = JSON.parse(preview.stdout);
  assert.equal(report.status.server, 'upgradeable');
  assert.equal(report.diff.operations[0].op, 'replace');
  assert.match(report.nextCommand, /mcp install --client cursor/);

  const applied = spawnSync(process.execPath, ['apps/cli/oaf.mjs', 'mcp', 'install', '--client', 'cursor', '--home', home, '--root', root, '--apply', '--confirm', report.planFingerprint, '--format', 'json'], { encoding: 'utf8' });
  assert.equal(applied.status, 0, applied.stderr);
  assert.equal(JSON.parse(readFileSync(configPath, 'utf8')).mcpServers.oaf.args[5], 'native');

  writeLegacy();
  const uninstall = spawnSync(process.execPath, ['apps/cli/oaf.mjs', 'mcp', 'uninstall', '--client', 'cursor', '--home', home, '--format', 'json'], { encoding: 'utf8' });
  assert.equal(uninstall.status, 0, uninstall.stderr);
  assert.equal(JSON.parse(uninstall.stdout).status.server, 'installed');
});
...
test('MCP code.impact returns bounded locator-safe impact and rejects unsafe map inputs',()=>{
  ...
  const result=spawnSync(process.execPath,['apps/cli/oaf.mjs','mcp','server','--read-only','--engine','compatibility','--root',root,'--sqlite','.local/memory.sqlite','--stdio'],{encoding:'utf8',env,input});
  assert.equal(result.status,0,result.stderr);
  const responses=result.stdout.trim().split(/\n/u).map((line)=>JSON.parse(line));
